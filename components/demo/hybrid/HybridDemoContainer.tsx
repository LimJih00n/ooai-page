...
import HybridAgentWorkflow from './HybridAgentWorkflow';
...
const initialLogs = [{ step: 'idle', message: '하이브리드 AI 워크플로우를 시작하려면 "데모 시작" 버튼을 클릭하세요.' }];

export default function HybridDemoContainer() {
  ...
  useEffect(() => {
    ...
    if (currentStep >= hybridSteps.length) {
      setIsRunning(false);
      setIsComplete(true);
      return;
    }
    ...
    const timer = setTimeout(() => {
      ...
      if (currentStep < hybridSteps.length - 1) {
        const nextStep = hybridSteps[currentStep + 1].id;
        let initialMessage = '';
        ...
        if(initialMessage) setLogs(prev => [...prev, { step: hybridSteps[currentStep + 1].id.split('_')[0].toUpperCase(), message: initialMessage }]);
      }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold">하이브리드 AI 워크플로우 데모</h2>
